...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for query in searchers:
            results = cr.works(query=query, filter={'has-abstract': True}, limit=1000)
            ...
            for item in results['message']['items']:
                ...
                try:
                    ...
                    if created:
                        logger.info(f'{Fore.GREEN}New paper: {query} - {article_data["title"]}{Style.RESET_ALL}')
                    else:
                        logger.info(f'{Fore.BLUE}Updated existing news: {query} - {article_data["title"]}{Style.RESET_ALL}')
                except:
